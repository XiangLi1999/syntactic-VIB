...
@Model.register("biaffine_parser_VIB")
class BiaffineDependencyParser(Model):
    ...
    def __init__(self,
        ...
        encoder = PytorchSeq2SeqWrapper(torch.nn.LSTM(tag_dim, options.lstm_dims, batch_first=True, bidirectional=True))
    ...
    def _construct_loss(self,
        ...
        # print('head_indices is, head_indices', head_indices)
...
if __name__ == '__main__':
    ...
    # data_loader = Data_Loader(None, path=sys.argv[1])
    # # embed = torch.nn.Embedding(len(data_loader.word_dict), 100)
    # # embed = Seq2SeqEncoder(embed)

    # vocab = Vocabulary({"counter": data_loader.word_count, "min_count": 2})
    EMBEDDING_DIM = 6
    HIDDEN_DIM = 6
    ...
    # encoder = torch.nn.LSTM(100, 50)
    model = BiaffineDependencyParser(None,tag_dim=10 )
    model.forward(data_loader.processed_sent[0:args.batch_size])
    # vocab: Vocabulary,
